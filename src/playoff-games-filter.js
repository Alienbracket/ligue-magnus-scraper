...
// Load existing playoff-todays-games to preserve all fields except scores/periods
// This prevents data "toggling" when the scraper temporarily fails or returns incomplete data
let existingTodaysGames = {};
try {
  const existingTodaysPath = path.join(__dirname, '../output/playoff-todays-games.json');
...
// Create today's games output (use processed games with vs_match and period)
const todaysOutput = {
  type: 'playoff-todays-games',
...
fs.writeFileSync(
  path.join(outputDir, 'playoff-todays-games.json'),
  JSON.stringify(todaysOutput, null, 2)
);

fs.writeFileSync(
  path.join(outputDir, 'playoff-upcoming-games.json'),
...
if (processedTodaysGames.length === 0) {
  console.log('No games scheduled for today.\n');
} else {
  processedTodaysGames.forEach((game) => {
...
console.log('='.repeat(60));
console.log('UPCOMING PLAYOFF GAMES');
console.log('='.repeat(60));
...
console.log('='.repeat(60));
console.log(`\nSaved to:`);
console.log(`  - ${path.join(outputDir, 'playoff-todays-games.json')} (${processedTodaysGames.length} games)`);
console.log(`  - ${path.join(outputDir, 'playoff-upcoming-games.json')} (${upcomingGames.length} games)`);
